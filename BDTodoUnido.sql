...
/*Modificaciones de eventos(andrea)*/
...
ADD COLUMN IF NOT EXISTS fecha_inscripcion TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW(),
ADD COLUMN IF NOT EXISTS fecha_confirmacion TIMESTAMP WITHOUT TIME ZONE,
ADD COLUMN IF NOT EXISTS posicion_lista_espera INTEGER;


-- Actualizar datos existentes
UPDATE soyucab.miembro_participa_evento 
SET tipo_participacion = 
    CASE estado_participacion
        WHEN 'confirmado' THEN 'confirmado'::soyucab.tipo_participacion_evento
        ELSE 'inscrito'::soyucab.tipo_participacion_evento
    END
WHERE tipo_participacion IS NULL;
...
-- Agregar constraints
ALTER TABLE soyucab.miembro_participa_evento 
ADD CONSTRAINT ck_confirmacion CHECK (
    fecha_confirmacion IS NULL
    OR fecha_confirmacion >= fecha_inscripcion
);

-- Crear tabla evento_dependencia para relacionar eventos con Dependencia UCAB
...
-- Crear tabla evento_organizacion para relacionar eventos con Organizaciones Asociadas
...
-- PRIMERO eliminar la función existente con su firma exacta
DROP FUNCTION IF EXISTS soyucab.resumen_gestion_eventos();

-- LUEGO crear la nueva función
CREATE OR REPLACE FUNCTION soyucab.resumen_gestion_eventos()
RETURNS TABLE (
    fecha_evento DATE,
    titulo_evento VARCHAR,
